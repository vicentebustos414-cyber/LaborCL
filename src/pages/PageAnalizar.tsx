...
import { Upload, X, Bot, Scale, Loader2, Maximize2, History } from 'lucide-react'
import { guardarAnalisis } from '@/lib/historialContratos'
...
function ModalPantallaCompleta({ titulo, resultado, onClose }: FullscreenProps) {
  return (
    <div className="fixed inset-0 z-50 bg-[#0D0D0D]/95 backdrop-blur-sm flex flex-col">
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b border-white/[0.07] flex-shrink-0">
        <div>
          <p className="text-xs text-[#6B6560] uppercase tracking-widest mb-1">Análisis del Contrato</p>
          <h2 className="font-serif text-lg font-semibold text-[#F0EDE8] truncate max-w-xl">{titulo}</h2>
        </div>
...
export function PageAnalizar({ onNavigate }: Props) {
  ...
  const clearFile = () => { setFile(null); setEstado('idle'); setResultado(''); setGuardado(false) }
  ...
  const extraerTextoPDF = async (f: File): Promise<string> => {
  ...
  return (
    <>
      {fullscreen && resultado && (
        <ModalPantallaCompleta
          titulo={file?.name ?? 'Contrato'}
          resultado={resultado}
          ...
          <div className="space-y-5">
            <Card className="p-7 space-y-4">
              ...
              >
                ...
                <input
                  ...
                  className="hidden"
                  onChange={e => { e.target.files?.[0] && setFile(e.target.files[0]); setEstado('idle'); setResultado('') }}
                />
              </div>
              {file && (
            ...
            {onNavigate && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-sm text-[#6B6560] hover:text-[#A09A93] transition-colors py-2"
              >
                <History size={14} />
                Ver historial de análisis anteriores
              </button>
            )}
          </div>
          ...
          <div className="space-y-5">
            <Card className="p-7 min-h-[340px]">
              <div className="flex items-center justify-between mb-5">
                <div className="text-sm font-semibold font-serif text-[#F0EDE8]">Análisis del Contrato</div>
                {estado === 'done' && (
                  <div className="flex items-center gap-2">
                    {guardado && (
